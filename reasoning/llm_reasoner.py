...
class LLMReasoner(Reasoner):
    def __init__(self, model=None, base_url="http://localhost:11434/api/generate", retries=3):
        if model is None:
    ...
    def think(self, input: str, memory: Memory, tools: ToolManager, interface=None, event_bus=None) -> dict:
        """
        ...
        try:
            ...
            def stream_callback(line):
                ...
                if event_bus:
                    event_bus.emit("on_llm_stream_line", {"line": line})
            ...
            if event_bus:
                event_bus.emit("on_llm_response", {
                    "input": input,
                    "prompt": prompt,
            ...
            try:
                ...
                try:
                    fallback_response = self._try_call_llm(fallback_prompt)
                    return {
                        "thoughts": f"Invalid LLM output, attempted rephrasing.",
                        "tool_name": "chat",
                        "tool_input": {
                    ...
                    return {
                        "thoughts": f"Double LLM failure: {str(inner_e)}",
                        "tool_name": "chat",
                        "tool_input": {
                            "response": f"Raw LLM output: {response_str.strip()}"
                        }
                    }
